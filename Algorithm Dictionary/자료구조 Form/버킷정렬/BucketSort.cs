...
namespace Algorithm_Dictionary.자료구조_Form.버킷정렬
{
    public partial class BucketSort : Form
    {
        ...
        private void BucketSort_Load(object sender, EventArgs e)
        {
            ...
            tb_sort.AppendText("키 값이 0과 1사이라고 가정할 때, n개의 키가 구간[0,1]을 n등분하고 각각 하나의 버킷으로 합니다.\r\n각 키를 크기에 따라 각 버킷에 분배한다면, 키가 구간 내에서 균등하게 분포한다 가정했으므로 하나의 버킷에는 하나의 키만 있을 확률이 높습니다.\r\n만약 한 버켓에 여러 키가 들어가게 된다면 같은 버킷에 속하는 키끼리는 간단한 정렬 알고리즘으로 정렬합니다.\r\n이 설명을 C코드로 보고 싶으시다면 코드보기 버튼을 눌러주세요.");
        }
