...
namespace Algorithm_Dictionary.자료구조_Form.링크드리스트
{
    public partial class LinkedList : Form
    {
        ...
        private void LinkedList_Load(object sender, EventArgs e)
        {
            ...
            tb_Delete.AppendText("다음으로는 연결리스트 해제화 함수입니다. 연결리스트를 해제화 하는 함수에서는 head에서 링크를 따라가면서 tail이 가리키는 노드를 해제한 후에 자신을 해제해야 합니다. 그리고 이 작업을 반복하면서 마지막 노드를 제거해야 합니다. 따라서 tail이 가리키는 노드를 제거한 후에 자신의 메모리를 해제해줍니다. 리스트 해제화 함수 코드를 보고싶으시면 아래 코드보기 버튼을 눌러주세요.");

            tb_PushBack.AppendText("다음으로는 링크드리스트 순차보관하는 함수입니다. 링크드리스트에서 순차보관을 하려면 tail 앞에 자료를 보관하면 순차적으로 보관할 수 있습니다. 순차보관하는 함수의 코드를 보고싶으시면 아래 코드보기 버튼을 눌러주세요.");

            tb_HangNode.AppendText("다음으로는 노드연결 함수입니다. 새로운 노드 now의 prev를 pos의 prev로 설정합니다. 그리고 now의 next를 pos로 설정합니다. 또한 pos의 prev 노드의 next를 now로 설정합니다. 그리고 마지막으로 pos의 prev를 now로 설정합니다. 이 함수 코드를 보고싶으시면 아래 코드보기 버튼을 눌러주세요.");

            tb_insert.AppendText("다음으로는 원하는 노드앞에 삽입하는 함수입니다. 먼저 자료를 보관하는 노드를 생성해야 하고 입력 인자로 받은 노드 앞에 새로 생성한 노드를 연결해 줍니다. 그리고 나서 마지막으로 보관한 자료의 개수를 1 증가해주면 됩니다. 자세한 코드를 보고 싶으시면 아래 코드보기 버튼을 눌러주세요.");

            tb_Erase.AppendText("다음으로는 특정위치의 노드제거를 하는 함수입니다. 먼저 제거하기 전에 리스트 연결을 끊어 줘야합니다. 그리고 노드를 위해 할당한 메모리를 해제하고 보관한 자료 개수를 -1 해주면 함수는 끝납니다. 이 함수 코드를 보고싶으시면 아래 코드보기 버튼을 눌러주세요.");
        }
