...
namespace Algorithm_Dictionary.자료구조_Form.계수정렬
{
    public partial class CountingSort : Form
    {
        ...
        {
            tb_description.AppendText("카운팅 정렬을 항복들의 순서를 결정하기 위해 집합에 각 항목이 몇 개씩 있는지 작업을 하면서 선형 시간에 정렬하는 효율적인 알고리즘입니다.\r\n또한, 속도가 빠르면서 안정적입니다.\r\n시간 복잡도 O(n)을 가지는 알고리즘입니다.하지만 제한 사항이 있습니다.\r\n정수나 정수로 표현할 수 있는 자료에 대해서만 동작을 한다는 제한이 있습니다.\r\n카운트들을 위한 충분한 공간을 할당하려면 집합내의 큰 정수를 알아야합니다.");
            tb_sort.AppendText("다음은 카운팅정렬의 수행과정입니다.\r\n첫 번째로는 최대값만큼 배열을 만듭니다.\r\n두번째로는 입력받은 값을 세서 만든 배열에 넣고 누적합니다.\r\n세 번째로는 입력받은 배열의 인덱스 값들을 순서대로 배열의 인덱스로 가게 합니다.\r\n마지막으로는 배열의 인덱스의 값을 하나 감소시킨 값을 정렬 시킬 배열에 넣어줌으로 정렬이 끝이 납니다.\r\n이 과정들을 C 코드로 보고 싶으시다면 아래 코드보기 버튼을 눌러주세요.");
        }
    }
}
